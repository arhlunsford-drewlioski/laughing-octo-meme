...
static func full_roster() -> Array[GoblinData]:
	## Fallback 10-goblin starter roster. One keeper, two defenders, three
	## midfielders, two attackers, two chaos goblins.
	return [
		#                         name, personality, position, SHO SPD DEF STR HP  CHA
...
static func opponent_roster() -> Array[GoblinData]:
	return [
		...
		make_goblin("Thudwick", "Runs into things on purpose.", "defender", 2, 3, 6, 6, 5, 4),
		...
		make_goblin("Krunk", "Big and angry.", "attacker", 6, 5, 3, 5, 4, 3),
		make_goblin("Snikkit", "Small and sneaky.", "chaos", 4, 7, 2, 2, 3, 8),
	]
...
const _FACTION_OPPONENTS: Dictionary = {
	1: {  # GILDED_CODEX — tactical, midfield-heavy
		...
		"templates": [
			["keeper",     1, 2, 6, 7, 5, 2],
			["defender",   2, 4, 6, 4, 5, 2],
			["defender",   2, 3, 6, 5, 5, 2],
			["midfielder", 3, 4, 5, 4, 5, 3],
			["midfielder", 4, 5, 3, 3, 4, 5],
			["attacker",   6, 5, 2, 3, 4, 3],
			["attacker",   5, 5, 2, 4, 3, 4],
			["chaos",      5, 6, 1, 2, 3, 7],
		],
	},
	2: {  # MIDNIGHT_SKULK — counter-attack, high chaos
		...
		"templates": [
			["keeper",     1, 2, 5, 7, 5, 3],
			["defender",   2, 5, 6, 4, 4, 3],
			["defender",   2, 3, 6, 5, 4, 5],
			["midfielder", 3, 5, 5, 3, 4, 4],
			["attacker",   6, 5, 2, 3, 4, 6],
			["chaos",      4, 7, 2, 2, 3, 8],
			["chaos",      5, 6, 1, 3, 3, 8],
			["chaos",      4, 6, 2, 2, 3, 9],
		],
	...
	3: {  # IRONCLAD_BASTIONS — defensive wall
		...
		"templates": [
			["keeper",     1, 2, 7, 8, 6, 1],
			["defender",   2, 2, 7, 7, 6, 2],
			["defender",   2, 4, 7, 5, 6, 2],
			["defender",   2, 3, 6, 6, 5, 4],
			["midfielder", 3, 3, 5, 5, 6, 2],
			["midfielder", 2, 4, 5, 4, 7, 2],
			["attacker",   5, 2, 3, 7, 6, 3],
			["attacker",   4, 3, 3, 6, 5, 3],
		],
	},
	4: {  # SCREAMING_TIDE — aggressive pressing
		...
		"templates": [
			["keeper",     1, 3, 4, 6, 5, 3],
			["defender",   3, 6, 4, 3, 4, 3],
			["midfielder", 3, 6, 4, 4, 5, 3],
			["midfielder", 4, 5, 3, 4, 4, 4],
			["attacker",   7, 6, 2, 4, 3, 4],
			["attacker",   6, 4, 1, 6, 4, 5],
			["chaos",      5, 7, 2, 3, 3, 7],
			["chaos",      6, 6, 1, 2, 3, 9],
		],
	},
	5: {  # THUNDERING_MAW — brute strength
		...
		"templates": [
			["keeper",     1, 2, 5, 8, 6, 2],
			["defender",   2, 2, 6, 7, 7, 2],
			["defender",   3, 3, 5, 7, 5, 5],
			["midfielder", 3, 4, 4, 6, 5, 3],
			["attacker",   6, 4, 2, 6, 4, 4],
			["attacker",   6, 3, 3, 8, 5, 3],
			["attacker",   5, 3, 2, 7, 5, 5],
			["chaos",      5, 5, 1, 6, 4, 8],
		],
	},
}

static func generate_opponent_roster(_faction: int = 0) -> Array[GoblinData]:
	## Generate 6 goblins for an opponent. Picks a random style bucket internally.
	## (faction param is legacy and ignored - kept for API compat.)
	var bucket_keys: Array = _FACTION_OPPONENTS.keys()
	var bucket: int = bucket_keys[randi() % bucket_keys.size()]
	var templates: Dictionary = _FACTION_OPPONENTS[bucket]
...
const _RECRUIT_POSITIONS: Array = [
	"keeper", "defender", "defender", "midfielder", "midfielder",
	"attacker", "attacker", "chaos",
]
...
static func generate_recruit() -> GoblinData:
	...
	return make_goblin(gob_name, personality, pos,
		...
		stats["strength"], stats["health"], stats["chaos"], 0)
